part of 'details_cubit.dart';

class DetailsUserRecipeCubit extends _DetailsCubit {
  DetailsUserRecipeCubit(this._appRepository) : super();

  final AppRepository _appRepository;

  @override
  Future<void> fetchData({
    required String externalId,
    String? membershipExternalId,
  }) async {
    try {
      final userRecipe = await _appRepository.getUserRecipe(
        externalId,
        membershipExternalId,
      );
      final fdaRDIs = await _appRepository.getConfigNutrition();
      final nutritionPreferences =
          await _appRepository.getNutritionPreferences();
      emit(
        state.copyWith(
          status: DetailsStatus.requestSuccess,
          userRecipe: userRecipe,
          fdaRDIs: fdaRDIs,
          nutritionPreferences: nutritionPreferences,
          selectedPortion: userRecipe.defaultPortion,
          quantity: userRecipe.defaultQuantity,
        ),
      );
    } on GetUserRecipeFailure catch (e) {
      emit(
        state.copyWith(
          status: DetailsStatus.requestFailure,
          errorMessage: e.message,
        ),
      );
    }
  }

  Future<void> delete({
    required String externalId,
    String? membershipExternalId,
  }) async {
    emit(state.copyWith(status: DetailsStatus.deleteRequestSubmitted));
    try {
      await _appRepository.deleteUserRecipe(
        externalId: externalId,
        membershipExternalId: membershipExternalId,
      );
      emit(state.copyWith(status: DetailsStatus.deleteRequestSuccess));
    } on DeleteUserRecipeFailure catch (e) {
      emit(
        state.copyWith(
          status: DetailsStatus.requestFailure,
          errorMessage: e.message,
        ),
      );
    }
  }
}
