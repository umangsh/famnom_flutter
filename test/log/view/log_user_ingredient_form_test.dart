...
class MockLogUserIngredientCubit extends MockCubit<LogState>
    implements LogUserIngredientCubit {}
...
void main() {
  ...
  late LogUserIngredientCubit logCubit;
  ...
  group('LogUserIngredientForm', () {
    setUp(() {
      appRepository = MockAppRepository();

      logCubit = MockLogUserIngredientCubit();
      ...
      when(
        () => logCubit.logUserIngredient(
          externalId: constants.testUUID,
          serving: userIngredient.defaultPortion!.externalId,
        ),
      ).thenAnswer((_) async {});
    ...
    group('calls', () {
                  ...
                  children: [
                    BlocProvider.value(
                      ...
                      child: BlocProvider.value(
                        value: logCubit,
                        child: const LogUserIngredientForm(
                          externalId: constants.testUUID,
                        ),
                      ),
                    )
      ...
      testWidgets('logUserIngredient when log button is pressed',
          (tester) async {
        when(() => logCubit.state).thenReturn(
          LogState(
            mealType: Dropdown.pure(constants.mealTypes[0]['id']!),
